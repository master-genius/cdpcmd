...
let outWarning = (text, errname = 'Warning') => {
  setTimeout(() => {
    console.error(`    \x1b[2;31;47m ${errname}: ${text} \x1b[0m\n`);
  }, 1280);
};
...
class loader {
  ...
  constructor (options = {}) {
    ...
    for (let k in options) {
      ...
      if (k === 'mname') {
        ...
        this._fmtSubGroup();
        continue;
      } else if (k === 'loadModel' || k === 'directModel') {
  ...
  loadController (app) {
    if (app.service.__titbit_loader__) {
  ...
  loadMidware (app) {
    if (app.service.__titbit_loader__) {
      outWarning('您已经使用titbit-loader加载过中间件，多个titbit-loader实例会导致中间件多次加载或冲突，重复操作将会被终止。');
      return false;
    }
  ...
  loadGlobalMidware (app, m) {
    ...
    var makeOpts = (groupname = null) => {
      ...
      if (groupname) {
        ...
        op.group = `${this.config.prePath}${op.group}`;
      }

...
class CDPC {
  constructor(options={}) {
    ...
    // watch 自愈检查周期（毫秒）
    this.watchGuardInterval = 60 * 1000

    // state 文件保活周期（毫秒）：writeChildState 只在状态迁移时写，
    // 服务长稳运行时 state 文件会被 /tmp 年龄清理删除，需周期性重写保活。
    this.stateKeepaliveInterval = 300 * 1000
    ...
    // [FIX] watch 自愈：事件目录被外部清理（如 systemd-tmpfiles 对 /tmp 的年龄清理）
    // 后 inotify watch 会静默失效，命令通道随之死亡。这里记录 watcher 句柄与故障标记，
    // 由 _watchGuard 周期探测并重建。
    this._watchers = []
    this._watchBroken = false
    this._watchGuardTimer = null
    this._watchGuardRunning = false
    this._stateKeepaliveTimer = null
  ...
  async startWatch() {
    ...
    this._registerEventWatchers()

    if (!this._watchGuardTimer) {
      this._watchGuardTimer = setInterval(() => {
        this._watchGuard()
      }, this.watchGuardInterval)
    }
  }

  // [FIX] 挂载逻辑独立成方法，watch 失效后可由守护周期整体重建
  _registerEventWatchers() {
    this._watchBroken = false
    ...
    this.watchEvents.filter(x => {
      ...
      this._bindWatch(evt_dir, (evt, fname) => {
        // [FIX] 守护周期的活性探针文件：watch 句柄可能无 error 事件地静默失效，
        // 只有主动探测才能发现，发现失败后由 _watchGuard 重建。
        if (fname === '.probe') {
          try { fs.unlinkSync(`${evt_dir}/.probe`) } catch (err) {}
          this._probeCallback && this._probeCallback()
          return false
        }
     ...
     * */
    this._bindWatch(this.eventDir, (evt, fname) => {
  ...
  // [FIX] 统一挂载 watcher：出错（目录被删、句柄失效）时打标记，交给守护周期重建
  _bindWatch(target, handler) {
    try {
      let w = fs.watch(target, handler)

      w.on('error', err => {
        this.errorHandle(err, '--ERR-WATCH--')
        this._watchBroken = true
      })

      this._watchers.push(w)
    } catch (err) {
      this.errorHandle(err, '--ERR-WATCH-CREATE--')
      this._watchBroken = true
    }
  }
  ...
  // [FIX] 探测命令目录的 watch 是否仍然活着：写入探针文件，限时等待事件回调
  _probeWatchAlive(timeoutMs = 1000) {
    return new Promise(resolve => {
      let probeFile = `${this.eventDir}/start/.probe`
      let done = false
      let timer = null
      ...
      let finish = alive => {
        ...
        clearTimeout(timer)
        if (this._probeCallback === finish2) this._probeCallback = null
        resolve(alive)
      }

      let finish2 = () => finish(true)

      this._probeCallback = finish2
  ...
  /**
   * [FIX] watch 守护：周期检查事件目录与 watcher 是否存活。
   * 目录被外部清理（如 systemd-tmpfiles 按 10 天年龄清理 /tmp）时，
   * inotify watch 静默失效、命令通道随之死亡，这里负责重建目录并重新挂载；
   * 同时刷新子目录与 pids 文件的时间戳，避免按年龄被清理。
   */
  async _watchGuard() {
    if (this._watchGuardRunning) return
    this._watchGuardRunning = true

    try {
      let needRepair = this._watchBroken

      if (!needRepair) {
        for (let d of this.watchEvents) {
          try {
            fs.accessSync(`${this.eventDir}/${d}`)
          } catch (err) {
            needRepair = true
            break
          }
        ...
        if (!needRepair) {
          try {
            fs.accessSync(`${this.eventDir}/pids`)
          } catch (err) {
            needRepair = true
          }
        }
      ...
      if (!needRepair) {
        needRepair = !(await this._probeWatchAlive())
      }
      ...
      if (needRepair) {
        ...
        try {
          ...
          this.errorHandle(
            new Error(`event watch 重建: ${this.eventDir}`),
            '--WARN-WATCH-REPAIR--'
          )
        } catch (err) {
          this.errorHandle(err, '--ERR-WATCH-REPAIR--')
          this._watchBroken = true
        }
      }

      // 保活：刷新目录与 pids 文件的时间戳，长稳运行时不再被 /tmp 年龄清理删除
      let now = new Date()

      for (let d of this.watchEvents) {
        try { fs.utimesSync(`${this.eventDir}/${d}`, now, now) } catch (err) {}
      }

      try { fs.utimesSync(`${this.eventDir}/pids`, now, now) } catch (err) {}

      try {
        for (let f of fs.readdirSync(`${this.eventDir}/pids`)) {
          try { fs.utimesSync(`${this.eventDir}/pids/${f}`, now, now) } catch (err) {}
        }
      } catch (err) {}
    } finally {
      this._watchGuardRunning = false
  ...
  startChild(chk) {
    ...
    ch.on('exit', (code, sig) => {
        ...
        cur.restart !== 'remove' && this.writeChildState(cur)
  ...
  remove(name) {
    ...
    fs.unlink(this.appStateFile(chk), err => {
      this.debug && err && console.error(err)
    })
  ...
  async monitorStart() {
    ...
    // [FIX] state 文件保活：writeChildState 只在状态迁移时写，
    // 服务长稳运行时 state 文件会因 /tmp 年龄清理被删，导致 CLI 无法回读状态。
    if (!this._stateKeepaliveTimer) {
      this._stateKeepaliveTimer = setInterval(() => {
        if (this.notWatch) return

        for (let k of this.monitorChilds) {
          let ch = this.childs[k]
          ch && this.writeChildState(ch)
        }
      }, this.stateKeepaliveInterval)
    }
  }
  ...
  monitorStop() {
    ...
    if (this._stateKeepaliveTimer) {
      clearInterval(this._stateKeepaliveTimer)
      this._stateKeepaliveTimer = null
    }
  }

...
let tok = new titok({
    //算法，默认值aes-256-gcm
    //其他可选：aes-192-gcm, aes-256-cbc, sm4-cbc
    alg: 'aes-256-gcm',
    //表示有效期的秒数
    expires: 3600,
    ...
    //初始向量值，用于和最开始的数据块进行运算，长度16，GCM类算法是12。
    //这个仅仅用于测试，实际要使用随机向量。
    iv : 'qawsedrftgyhujie',
})
...
```javascript
//返回值： [IV].[TOKEN]
TitbitToken.prototype.makeAccessToken(info, id=null, key=null)
```
...
```javascript
//返回值和verify一致
TitbitToken.prototype.verifyAccessToken(info)
```

...
function _checkAppName(name) {
  ...
  //16个字符用于名字，4个字符用于编号，8个字符作为备用空间，绝对足够使用。
  return (/^[a-z1-9_][a-z0-9_-]{0,28}$/i).test(name)
}
...
class CDPC {
  ...
  loadConfig(filename='', reload=false) {
    ...
    /**
     * 重新加载模式，会遇到一些问题：
     * - 应用的name改了，或者是文件路径有所更改。
     * - 实际的文件其实已经不存在了，其实就是这个服务的配置文件已经不存在了。
     * 所有这些情况，都没有什么更好的办法，直接全部重启服务即可。
     */
    try {
      // [FIX] reload 时先快照当前所有受管服务的 name。
      // runChilds 完成后，找出已从配置中删除的服务并 safeRemove，
      // 避免这些服务因无人管理而成为孤儿进程。
      let namesBefore = reload ? new Set(Object.keys(this.appName)) : null

      this.runChilds(r.data, reload)

      if (reload && namesBefore) {
        // 构建新配置中实际存在的 name 集合，截断逻辑与 checkConfig 保持一致
        let namesAfter = new Set(
          r.data
            .filter(cfg => cfg.name)
            .map(cfg => cfg.name.length > 28 ? cfg.name.substring(0, 28) : cfg.name)
        )

        for (let oldName of namesBefore) {
          if (!namesAfter.has(oldName)) {
            this.debug && console.log(`[reload] 服务 ${oldName} 已从配置中移除，执行 safeRemove`)
  ...
  checkConfig(cfg, reload=false) {
    ...
    if (!cfg.name) {
      ...
      if (cfg.name.length > 28) {
        cfg.name = cfg.name.substring(0, 28)
      }

      if (!_checkAppName(cfg.name)) {
        this.lastErrorInfo = 
          `命名不合法：${cfg.name}，支持字母数字下划线减号，并且以字母或数字开头，长度不超过28。`
  ...
  makeName(chk) {
    ...
    let args = Array.isArray(chk.args) ? [...chk.args] : []

    args.sort((a,b) => {return a > b ? 1 : -1})
  
    let data = `${chk.command}${args.join('::')}`
    h.update(data)
    let hname = h.digest('hex')
    let final_name = hname.substring(0, 18)

    if (this.appName[final_name]) {
      return this.serialName(final_name + '_')
    }
  ...
  async writeChildState(chk) {
    ...
    str_state = `${str_state} ${chk.child ? (chk.child.pid||'0') : '0'} ${(new Date()).toLocaleString().replaceAll('/', '-')}`
  ...
  /**
   * 更新子进程的command，这个操作目的在于如果旧的命令被删除，则需要自动更新。
   * */
  setChildCommand(name, command) {
    let ap = this.find(name)
    if (!ap) return false
    let old_ck = `${ap.command}\x00${ap.args.join('\x00')}`
    let ck = `${command}\x00${ap.args.join('\x00')}`
    ap.command = command

    delete this.childs[old_ck]
    this.childs[ck] = ap
    this.appName[name] = ck
  
    return ap
  }

  /**
   * [FIX] 统一清理旧进程：停止旧子进程、清理 monitorChilds、清理 relationAfter 中对该 name 的引用。
   * 在 tryMakeChild 进入主逻辑前调用，处理"name 相同但 ck 变化"的场景（改文件名/路径）。
   * @param {string} name  app name
   * @param {string} newCk 新的 cmdline key
   */
  _cleanupOldByName(name, newCk) {
    let old_ck = this.appName[name]
    if (!old_ck || old_ck === newCk) return

    let old_chk = this.childs[old_ck]
    ...
    if (old_chk) {
      ...
      // 清理 monitorChilds
      let ind = this.monitorChilds.indexOf(old_ck)
      if (ind >= 0) this.monitorChilds.splice(ind, 1)

      delete this.childs[old_ck]
    }

    // 清理 relationAfter 中对该 name 的旧引用，避免 reload 后累积重复依赖
  ...
  /**
   * [FIX] 检查并处理 detached 进程的恢复。
   * 在 startChild 的 spawn 之前调用。
   * @param {object} chk 
   * @returns {boolean} true 表示旧进程仍在运行且不应启动新进程
   */
  _recoverDetachedProcess(chk) {
    ...
    // 检查旧进程是否仍在运行
    try {
      process.kill(oldPid, 0)  // 信号0：仅检测进程是否存在，不发送信号
    } catch (err) {
      // ESRCH: 进程不存在；EPERM: 无权限但进程存在
      if (err.code === 'ESRCH') {
        // 进程已不存在，清理 PID 文件，允许启动新进程
        try { fs.unlinkSync(pidFile) } catch(e) {}
        return false
      }

      if (err.code === 'EPERM') {
        // 无权限操作该进程（非root场景），进程仍在运行
        this.debug && console.error(
          `[detached-recover] 进程 ${oldPid} 仍在运行但无权限操作 (${chk.name})`
        )
        chk.cause = `EPERM|DETACHED-RECOVER|旧的detached进程(PID:${oldPid})仍在运行，无权限操作。`
        return true
      }

      // 其他错误，保守处理：清理 PID 文件，允许启动
      try { fs.unlinkSync(pidFile) } catch(e) {}
      return false
    }
    ...
    if (chk.force) {
      ...
      this.debug && console.log(
        `[detached-recover] force 模式，终止旧的 detached 进程 PID:${oldPid} (${chk.name})`
      )
      try {
    ...
    // 非 force 模式：不启动新进程
    this.debug && console.log(
      `[detached-recover] 旧的 detached 进程 PID:${oldPid} 仍在运行，跳过启动 (${chk.name})`
    )
    chk.cause = `RUNNING|DETACHED-RECOVER|旧的detached进程(PID:${oldPid})仍在运行，不属于当前cdpc管理。`
    return true
  }
  ...
  tryMakeChild(cfg, reload=false) {
    ...
    if (this.childs[ck]) {
      if (reload) {
        // [FIX] reload 时必须先 kill 旧进程，不能把旧 child 引用带入新对象。
        let old_chk = this.childs[ck]
        if (old_chk.child) {
          old_chk.restart = 'none'
          try {
            this.killAllChilds(old_chk.child.pid, 'SIGTERM')
        ...
        delete this.childs[ck]
        // child 不再传递给新对象，让 startChild 重新 spawn
      } else if (this.childs[ck].state !== this.state.EXIT) {
        return this.childs[ck]
      }
    }
  ...
  startChild(chk) {
    ...
    if (chk.only) {
      ...
      try {
        ...
        if (cmds.length > 0) {
          if (chk.force) {
            process.kill(cmds[0].pid, 'SIGKILL')
          } else {
    ...
    // [FIX] detached 进程恢复检查：在 spawn 之前检查旧的 detached 进程是否仍在运行
    ...
    ch.on('spawn', (stdout, stderr) => {
      let cur = this.appName[chk.name] ? (this.childs[ this.appName[chk.name] ] || chk) : chk
  ...
  remove(name) {
    ...
    delete this.relationAfter[name]
  ...
  async _getOneProcInfo(ch, kname) {
    ...
    if (ch && ch.state !== this.state.EXIT && ch.child) {
      parse_linux_proc_stat(ch.child.pid, ch.loadinfo[kname])
              .then(proc => {
        ...
        if (ch.monitorNetData) {
          if (ch.loadinfo.net) {
            parse_linux_pid_net(ch.child.pid, ch.loadinfo.net)
          } else {
            parse_linux_pid_net(ch.child.pid).then(netdata => {
  ...
  showLoadInfo() {
    ...
    if (this.loadInfoFile && this.loadInfoFile !== '--mem') {
      fs.writeFile(this.loadInfoFile,
          ...
          err => {
              err && this.errorHandle(err, '--ERR-LOADINFO--')
          })
    ...
    if (loadType === 'json') {
        ...
        for (let ck of this.monitorChilds) {
            ...
            d.childs.push({
                ...
                pid: ch.state === this.state.EXIT ? -1 : ch.child.pid,
                net: ch.loadinfo.net,
                restart: ch.restart,

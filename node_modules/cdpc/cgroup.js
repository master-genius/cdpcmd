...
class cgroup {
  ...
  constructor() {
    ...
    this.defaultControl = ['cpu', 'memory', 'pids', 'io']
  ...
  async create(name, detail) {
    ...
    for (let x of this.defaultControl) {
      if (carr.indexOf(x) < 0) {
        await fsp.writeFile(cgrp_subtree_control, '+' + x, {flag: 'a'})
      }
    }
  ...
  async setMem(name, mem) {
    ...
    try {
      await fsp.writeFile(this.cgdir + '/' + name + '/memory.max', `${detail.memory}`)
      detail.memory = mem
      return true
    } catch (err) {
      return false

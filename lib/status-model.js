...
const { expandProcCmd } = require('./proccmd')
...
function detailLines(ch, opts={}) {
   ...
   * 进程列表上限。
   *
   * 不指定服务名时（看全表）列表必须封顶，否则一个几十进程的服务就把整屏
   * 冲掉了；而 `cdpc status <名字> -l` 是明确在查这一个服务，此时封顶只会
   * 妨碍排障——要的就是完整的进程树。
   *
   * 注意这只是**本地展示**的上限。cdpcd 侧还有一道 TREE_PROC_LIMIT=20
   * （cdpc 库里的模块常量，无开关），超出的部分根本没进 status 应答，
   * 只能靠 procsOmitted 如实说明，见下面的折叠行。
   */
  let procLimit = opts.fullProcs ? Infinity : PROC_LIST_LIMIT
  ...
  if (Array.isArray(ch.procs) && ch.procs.length > 1) {
    ...
    let list = ch.procs.slice(0, procLimit)

    list.forEach((p, i) => {
      let last = (i === list.length - 1) && ch.procs.length <= procLimit
      ...
      /**
       * daemon 把 cmd 截到 72 字符就发过来了，直接用的话表格加宽也看不到更多，
       * 折行更是永远不会触发。这里就地补读 /proc/<pid>/cmdline 还原完整命令；
       * 只对展开详情时列出的这几个进程做（PROC_LIST_LIMIT 上限 5 个），
       * 且命令没被截断时不产生任何读盘。
       */
      let cmdText = expandProcCmd(p.pid, p.cmd || p.comm || '')

      for (let ln of wrapCommand(prefix, cmdText, innerWidth, cont)) {
        lines.push(ln)
      }
    })
     ...
     */
    let rest = ch.procs.slice(procLimit)
    ...
    if (restCount > 0) {
      ...
      /**
       * 本地已经全列了却仍有遗漏，说明是 cdpcd 侧的进程树上限截的——
       * 这时候必须点明是谁截的，否则用户会以为 `status <名字> -l` 骗了他。
       */
      let why = (rest.length === 0) ? '（cdpcd 侧进程树上限）' : ''

      lines.push('   └─ ' + `其他 ${restCount} 个进程${why}  `
...
function buildRows(childs, { detail=false, widths=null, fullProcs=false } = {}) {
  let innerTextWidth = summaryInnerTextWidth(widths)
  let rows = []
  for (let ch of childs) {
    rows.push({ type: 'cells', cells: summaryCells(ch) })
    if (detail) {
      rows.push({ type: 'span', lines: detailLines(ch, { innerTextWidth, fullProcs }) })
    }
  }
  return rows
}

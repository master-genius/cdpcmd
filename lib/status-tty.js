...
function parseKey(chunk) {
  ...
  // chunk 内可能粘连多个事件：对首字符兜底匹配。
  // 但 ESC 开头的多字节是未识别的 escape 序列（鼠标滚轮、功能键、终端上报等），
  // 不能让它兜底匹配到 KEY_TABLE['\x1b'] = 'quit'，否则滚轮一动就退出。
  if (s.length > 1 && s[0] !== '\x1b' && KEY_TABLE[s[0]]) return KEY_TABLE[s[0]]
  return null
}

...
class loader {
    ...
    for (let k in cfiles) {
      try {
        ...
        if (cob.init && typeof cob.init === 'function') {
            if (this.config.initArgs) {
              cob.init(this.config.initArgs);
            } else {
              cob.init();
            }
        }
        ...
        outWarning(`load router file : ${k}\x1b[0m\n  \x1b[1;33m${err.message}`, 'Error', '\x1b[1;35m');
      }
    }
  ...
  setRouter (app, cob, cf) {
    ...
    if (cob.list !== undefined && typeof cob.list === 'function') {
      app.router.get(`${route_path}`, cob.list.bind(cob),{
    ...
    if (cob.patch !== undefined && typeof cob.patch === 'function') {
      app.router.patch(`${route_path}`, cob.patch.bind(cob),{
    ...
    if (cob.options !== undefined && typeof cob.options === 'function') {
      ...
      if (group === `${this.config.prePath}/`) {
        this._autoAddOptions(app, `${route_path}/*`, group);

      } else if (this.optionsCacheLog[group] === undefined) {
        this._autoAddOptions(app, `${group}/*`, group);
        this.optionsCacheLog[group] = true;
      }
    }
  ...
  _getGroupList () {
    ...
    try {
      ...
      console.error('获取全局所有分组失败，在separate模式，若获取失败会导致程序运行错误，故进程退出。');
      console.error(err);
      process.exit(1);
    }
  ...
  loadMidware (app) {
    ...
    this.config.separate && this._getGroupList();
  ...
  bindModelAttr (app) {
    if (!this.config.mname) {
        app.service.__model__ = app.service;
    } else {
      if (app.service[this.config.mnam] === undefined) app.service[this.config.mname] = {};

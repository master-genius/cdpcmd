...
const sockpath = require('./sockpath')
...
function describe(kind, sockFile, err) {
  switch (kind) {
    case 'down': {
      // 半升级状态（新 CLI + 仍在运行的旧版 daemon）要给出可操作的线索，
      // 否则用户看到的是"未运行"，而 daemon 明明还在跑
      let hint = sockpath.legacyHint(process.geteuid(), sockpath.currentHome())
...
class SockClient {
  ...
  async control(op, name, wantState = '', waitMs = 10000) {
    ...
    return this.waitState(name, wantState, waitMs, prevPid)
  }

  /**
   * 轮询等待服务达成目标状态（**不下发任何 op**）。
   *
   * 从 control 里抽出来单独暴露：reload-force 的最后一步是"发完 load 之后
   * 等服务被重建出来"，而 load 不带 name、也不是控制类 op，套不进 control。
   * 轮询逻辑只此一份，避免两处实现漂移。
   *
   * @param {string} name
   * @param {string} wantState RUNNING / EXIT / PAUSE / REMOVED
   * @param {number} waitMs
   * @param {number|null} prevPid 非 null 时要求 pid 与之不同才算达成（restart 用）
   * @returns {object} {ok, state, ms} 或 {ok:false, error}
   */
  async waitState(name, wantState, waitMs = 10000, prevPid = null) {
    let start = Date.now()
    let lastRow = null

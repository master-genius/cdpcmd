...
class CDPC {
  ...
  /**
   * [包C] 把子进程的 stdout/stderr 采集到 chk.logFile。
   * 写入流的生命周期由 cdpc 管理：spawn 时打开，exit 时关闭。
   */
  _attachLogFile(chk, ch) {
    try {
      this._rotateLogFile(chk.logFile)
      let ws = fs.createWriteStream(chk.logFile, {flags: 'a', mode: 0o644})
      chk._logStream = ws
      ws.on('error', () => { chk._logStream = null })
      ws.write(`\n===== [${(new Date()).toLocaleString()}] ${chk.name} pid=${ch.pid} 启动 =====\n`)
      ch.stdout.pipe(ws, {end: false})
      ch.stderr.pipe(ws, {end: false})
    } catch (err) {
      this.errorHandle(err, '--ERR-ATTACH-LOGFILE--')
    }
  }

  /**
   * [包C] 单备份按大小轮转：日志超过 maxSize 就改名为 .1（覆盖旧备份）。
   */
  _rotateLogFile(logfile, maxSize = 5 * 1024 * 1024) {
    try {
  ...
  tryMakeChild(cfg, reload=false) {
    ...
    if (!this.childs[cfg.name]) {
      this.childs[cfg.name] = {
        ...
        // [包C] 子进程 stdout/stderr 采集目标文件 + 写入流句柄
        logFile: (typeof cfg.logFile === 'string') ? cfg.logFile : '',
        _logStream: null,
  ...
  startChild(chk) {
    ...
    // [包C] logFile：把子进程 stdout/stderr 重定向为 pipe 以便采集
    if (chk.logFile) {
    ...
    // [包C] 采集 stdout/stderr 到 logFile
    if (chk.logFile && ch.stdout && ch.stderr) {
      this._attachLogFile(chk, ch)
    }
    ...
    ch.on('exit', (code, sig) => {
        ...
        // [包C] 关闭 logFile 写入流（延迟一点让管道里残留数据落盘）
        if (cur._logStream) {
          let ws = cur._logStream
          cur._logStream = null
          setTimeout(() => { try { ws.end() } catch(e) {} }, 200)
        }

...
/**
 * 东亚宽字符（中日韩文字、全角标点、假名、韩文等）在终端里占 2 列，
 * 按 length 算会让含中文的行把右边框推出去（表格错位）。
 * 这里按 Unicode East Asian Width 的常用区间判定。
 */
function charWidth(cp) {
  if (cp >= 0x1100 && (
      cp <= 0x115f                                  // Hangul Jamo
      || cp === 0x2329 || cp === 0x232a
      || (cp >= 0x2e80 && cp <= 0xa4cf && cp !== 0x303f)  // CJK 部首/假名/注音/CJK
...
function visibleWidth(s) {
  let text = String(s).replace(/\x1b\[[0-9;]*m/g, '')
  let w = 0
...
function truncateByVisible(s, maxVisible) {
  ...
  while (i < s.length && visible < maxVisible) {
    if (s[i] === '\x1b' && s[i + 1] === '[') {
      ...
      // 按码点推进：宽字符占 2 列，且不能把代理对切成两半
      let cp = s.codePointAt(i)
      let chLen = cp > 0xffff ? 2 : 1
      let w = charWidth(cp)

      if (visible + w > maxVisible) break

      out += s.substr(i, chLen)
      visible += w
      i += chLen
    }
  }

...
| onceCommand | 作为命令执行一次 | 否 | true或false。相当于设定了autoRemove为true、restart为count、restartLimit为0。 |
...
| configCallback | 配置文件加载后回调函数，不设置则不执行。 | 函数，接收参数为一个数组，是所有读取的子进程配置，但是还没有进行格式化处理，是原始的导入模块后的项。 | 函数。 |
| disabledCallback | 运行子进程之前的回调函数，接收参数为一个配置对象，是格式化后的配置对象。 | 函数。 |
